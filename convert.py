...
class Metadata:
  ...
  def to_dict(self):
    ...
    if self.arch in SUPPORTED_ARCHITECTURES:
...
def load_weights(model_files, dtype_str, metadata, tie_word_embeddings):
  ...
  dtype = {"fp32": torch.float32, "fp16": torch.float16, "fp8": torch.float8_e5m2, 'bf16': torch.bfloat16}[dtype_str]
