...
inline float bf16_to_float(bf16_t x) {
    uint32_t u = static_cast<uint32_t>(x) << 16;  // put bf16 in upper 16 bits
    float f;
    std::memcpy(&f, &u, sizeof(f));               // safe bit-cast
    return f;
}

inline bf16_t float_to_bf16(float f) {
    uint32_t u;
    std::memcpy(&u, &f, sizeof(u));               // get float bits
    return static_cast<bf16_t>(u >> 16);          // keep top 16 bits
}
...
#if defined(__AVX2__)
  // W (d,n) @ x (n,) -> xout (d,)
  assert(n % 16 == 0 && "n must be a multiple of 16 for this AVX2 path");
  int i;
#pragma omp parallel for private(i)
  for (i = 0; i < d; i++) {
    ...
    const bf16_t* wi = w + (size_t)i * n;

    for (int j = 0; j < n; j += 16) {
      // Load 16 bf16 = 256 bits
      __m256i w16 = _mm256_loadu_si256((const __m256i*)(wi + j));

      // Extract 2x128 lanes of 8 bf16 each
      __m128i w16_lo = _mm256_extractf128_si256(w16, 0);
      __m128i w16_hi = _mm256_extractf128_si256(w16, 1);

      // Zero-extend u16->u32 (8 lanes each), then shift left 16 to place bf16 in high bits of fp32
      ...
      // Reinterpret as float32 (now valid fp32 values)
...
#else
  // Portable scalar fallback (no vectorization).
  int i;
#pragma omp parallel for private(i)
  for (i = 0; i < d; i++) {
    float acc = 0.0f;
    const bf16_t* wi = w + (size_t)i * n;
    for (int j = 0; j < n; ++j) {
      // bf16 -> fp32: place bf16 in high 16 bits of a uint32, reinterpret as float
...
void Block::_block_cpu(
  ...
  if constexpr (std::is_same_v<T, bf16_t>) {
    ...
    bf16_matmul(s.q(), s.xb(), wq<T>(), c.dim, q_dim);
    bf16_matmul(s.k(), s.xb(), wk<T>(), c.dim, kv_dim);
    bf16_matmul(s.v(), s.xb(), wv<T>(), c.dim, kv_dim);
  } else {
...
#pragma omp parallel for private(h)
  ...
  if constexpr (std::is_same_v<T, bf16_t>) {
    bf16_matmul(s.hb(), s.xb2(), wo<T>(), q_dim, c.dim);
  } else {
  ...
  if (c.n_experts > 0) {
    if constexpr (std::is_same_v<T, bf16_t>) {
      bf16_matmul(s.moe_weights(), s.xb(), moegate<T>(), c.dim, c.n_experts);
    } else {
      matmul(s.moe_weights(), s.xb(), moegate<T>(), c.dim, c.n_experts);
    }
    moe_gate(s.active_experts_weights(), s.active_experts(),
             s.moe_weights(), c.n_experts, c.n_experts_active);
  } else {
  ...
  for (int k = 0; k < (c.n_experts > 0 ? c.n_experts_active : 1); ++k) {
    ...
    if constexpr (std::is_same_v<T, bf16_t>) {
      bf16_matmul(s.hb(), s.xb(), w1<T>() + expert_index * expert_size, c.dim, c.hidden_dim);
      bf16_matmul(s.hb2(), s.xb(), w3<T>() + expert_index * expert_size, c.dim, c.hidden_dim);
    } else {
    ...
    if constexpr (std::is_same_v<T, bf16_t>) {
      bf16_matmul(s.xb2(), s.hb(), w2<T>() + expert_index * expert_size, c.hidden_dim, c.dim);
    } else {
...
// void matmul_cpu(float* xout, float* x, bf16_t* w, int n, int d) {
//   bf16_matmul(xout, x, w, n, d);
// }
...
void Model::_copy_embedding(InferenceState& s, int token) {
  ...
  switch (c.weight_dtype) {
    ...
    case DType::BF16: {
      ...
      for (int i = 0; i < c.dim; i+=1) {
        s.x()[i] = bf16_to_float(emb[token * c.dim + i]);
      }
      break;
    }
    default: {
...
void Model::_forward_cpu(InferenceState& s, int token, int pos, InferenceMode mode) {
  ...
  switch (c.weight_dtype) {
    ...
    case DType::BF16: {
    bf16_matmul(s.logits(), s.x(), static_cast<bf16_t*>(wcls), c.dim, c.vocab_size);
    break;
    }
    default: {
      assert(false && "unsupported weight dtype");
